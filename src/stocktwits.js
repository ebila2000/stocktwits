...
const fetchstocktwitsbysymbol =
    ...
    const fetchstocktwitsbyuser =
    (user, proxyip, proxyport, callback) =>
    {

        if (proxyip === "0.0.0.0") { reqobj = { url: 'https://api.stocktwits.com/api/2/streams/user/' + encodeURIComponent(user) + '.json', json: true } }
        else { reqobj = { url: 'https://api.stocktwits.com/api/2/streams/user/' + encodeURIComponent(user) + '.json', json: true, proxy: `http://${proxyip}:${proxyport}`, }}
...
module.exports =
    {
        fetchstocktwitsbysymbol : fetchstocktwitsbysymbol,
        fetchstocktwitsbyuser: fetchstocktwitsbyuser
    }
